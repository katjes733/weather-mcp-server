...
describe("DailyForecastWeather", () => {
  ...
  it("getForecast throws if fetch not ok", async () => {
    const tool = createInstance();
    mockFetch.mockResolvedValue({ ok: false, status: 500 });
    await expect(
      tool["getForecast"]("https://api.weather.gov/gridpoints/PSR/171,48"),
    ).rejects.toThrow("Error fetching forecast data: 500");
  });

  it("getForecast returns forecast from URL", async () => {
    const tool = createInstance();
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({
    ...
    const result = await tool["getForecast"](
  ...
  it("getForecast throws if API returns no properties property", async () => {
    const tool = createInstance();
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({}),
  ...
  it("getForecast throws if API returns undefined forecastGridData", async () => {
    const tool = createInstance();
    mockFetch.mockResolvedValue({
      ok: true,
